...
def plot_power_breakdown_crossbar(computer: Computer):
    Ni = [N_pt, N_du, 4*math.sqrt(Nq)+1, 4*math.sqrt(Nq)+1, 1, math.sqrt(Nq/2), math.sqrt(Nq/2), math.sqrt(Nq/2), N_classical_comp, N_chill]
    Ni = [int(x) for x in Ni]
    computer.N_comp = Ni
    ...
    plt.savefig("Figures/Spin_qubits/spin_qubits_power_breakdown_crossbar.pdf")
    plt.close()
    print(computer.P, computer.P*3600*24/1000000)
...
def planar_vs_linear(computer):
    ...
    Ni = [N_pt, N_du, math.ceil(Nq*2/5), math.ceil(Nq*2/5), math.ceil(Nq*2/5), math.ceil(Nq/10), math.ceil(Nq/10), math.ceil(Nq/10), N_classical_comp, N_chill]
    Ni = [int(x) for x in Ni]
    computer.N_comp = Ni
    computer.list_components = computer.assemble()
    for D0 in D_values:
...
def crossbar_vs_planar_vs_linear(computer):
    D_values = np.arange(0, 2000, 5)
    EE_planar = []
    EE_crossbar_og = []
    EE_crossbar_red = []
    ...
    for D0 in D_values:
        ...
        if D0 == 500:
            ...
            print("2D", computer.energy_efficiency(T = T, algorithm=alg, N_sampl=1))
        EE_planar.append(computer.energy_efficiency(T = T, algorithm=alg, N_sampl=1))
    ...
    for D0 in D_values:
        ...
        if D0 == 500:
            circuit = Circuit(algorithm=alg, avg_diameter=computer.avg_diameter)
            print("Linear", computer.energy_efficiency(T = T, algorithm=alg, N_sampl=1))
        EE_linear.append(computer.energy_efficiency(T = T, algorithm=alg, N_sampl=1))
    ...
    Ni = [int(x) for x in Ni]
    computer.t_clock = 250e-9
    computer.t_meas = 10e-5
    computer.N_comp = Ni
    computer.list_components = computer.assemble()
    for D0 in D_values:
        alg = Algorithm(D=D0, eta=0.9)
        if D0 == 0:
            circuit = Circuit(algorithm=alg, avg_diameter=computer.avg_diameter)
            print("Crossbar", computer.energy_efficiency(T = T, algorithm=alg, N_sampl=1))
        EE_crossbar_og.append(computer.energy_efficiency(T = T, algorithm=alg, N_sampl=1))
    
    computer.t_clock = 10e-9
    computer.t_meas = 10e-6
    computer.N_comp = Ni
    for D0 in D_values:
        alg = Algorithm(D=D0, eta=0.9)
        if D0 == 0:
            circuit = Circuit(algorithm=alg, avg_diameter=computer.avg_diameter)
            print("Crossbar", computer.energy_efficiency(T = T, algorithm=alg, N_sampl=1))
        EE_crossbar_red.append(computer.energy_efficiency(T = T, algorithm=alg, N_sampl=1))


    ax.plot(D_values, EE_linear, label='Linear', color = colors[0])
    ax.plot(D_values, EE_planar, label='2D', color = colors[1])
    ax.plot(D_values, EE_crossbar_og, label=r'Crossbar ($t_{\rm{clock}}=250~\mathrm{ns}$, $t_{\rm{meas}}=100\;\mu\mathrm{s}$)', color = colors[1], linestyle='--')
    ax.plot(D_values, EE_crossbar_red, label=r'Crossbar ($t_{\rm{clock}}=10~\mathrm{ns}$, $t_{\rm{meas}}=10\;\mu\mathrm{s}$)', color = colors[1], linestyle='-.')
    ax.fill_between(D_values, EE_planar, EE_crossbar_red, color=colors[1], alpha=0.2)
    ax.text(400, 3.6e-1, r'$P^{\rm{2D}}-P^{\rm{crossbar}}=5.91\mathrm{kW}$', fontsize=10, color='black', rotation=-3.5)
    ax.set_xlabel(r"Pre-routing Circuit Depth, $D_{0}$")
    ax.set_ylabel(r"Energy Efficiency, $EE$ (computations/J)")
    ax.set_xlim(0, 1000)
    ax.set_ylim(2.4e-2, 3.7e0)
    ax.set_yscale('log')
    ax.legend(fontsize=11, loc='upper right', fancybox=False, edgecolor='black')
