...
dilution_unit, N_du = Component('Dilution Unit', 2000, 'Cooling'), 1
...
def plot_power_breakdown():
    computer = SolidStateComputer(components=components, N_comp=Ni, graph_type="Square", t_reset=t_reset_best, t_meas=t_meas, t_clock=t_g_best, Nq = 49)
    print(f"Total power: {computer.P} W")
...
def crossbar_vs_planar_vs_linear():
    ...
    Ni = [N_pt, N_du, N_DC_control, N_BB_control, N_MW_control, N_redout_lockin, N_lna, N_rt_amplifier, N_classical_comp, N_chill]
    Ni = [int(x) for x in Ni]
    computer_49 = SolidStateComputer(components=components, N_comp=Ni, graph_type="Square", t_reset=t_reset_best, t_meas=t_meas, t_clock=t_g_best, Nq = Nq)
    ...
    N_r = math.ceil(Nq/2)
    Nq = 49
    Ni = [N_pt, N_du, 2*Nq-1, 2*Nq-1, Nq, N_r, N_r, N_r, N_classical_comp, N_chill]
    Ni = [int(x) for x in Ni]
    ...
    Nq = 49
    N_dc = math.ceil(4*math.sqrt(2*Nq)+1)
    N_bb = math.ceil(4*math.sqrt(2*Nq)+1)
    N_mw = 1
    N_r = math.ceil(math.sqrt(2*Nq))
    Ni = [N_pt, N_du, N_dc, N_bb, N_mw, N_r, N_r, N_r, N_classical_comp, N_chill]
    Ni = [int(x) for x in Ni]
    ...
    ax.plot(D_values, EE_planar_49, label='Square', color = "yellowgreen")
    ...
    ax.text(400, 0.39, r'$P^{\rm{2D}}-P^{\rm{crossbar}}\approx 1\mathrm{kW}$', fontsize=10, color='black', rotation=-9)
...
def plot_D_and_Nsamples():
    computer = SolidStateComputer(components=components, N_comp=Ni, graph_type="Square", t_reset=t_reset_best, t_meas=t_meas, t_clock=t_g_best)
    ...
    for i, N_samples in enumerate(N_samples_values):
        ...
        N_pi_list = []
        for D0 in D_values:
            EE_list.append(computer.energy_efficiency(D0, alpha, N_samples=N_samples))
            N_pi_list.append(computer.N_pi(D0 = D0, alpha = alpha, t = 24*3600, N_samples=N_samples))
